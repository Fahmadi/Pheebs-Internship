...
async function imdb_crowler() {
    try {
      var movies = [];
      const $ = await rp(options);
      const href = [];
        console.log('Crowler Start')
      ...
      async function addDirectorAndStarsToMovie(movie) {
        ...
        const movie_page = {
          uri: `https://www.imdb.com//title/` + movie.imdbId,
          transform: function(body) {
            return cheerio.load(body);
          }
        };
      ...
      fs.writeFile("output.json", JSON.stringify(movies, null, 2), async function(err) {
        if (err) throw err;
        console.log("Crowler Complete");
      });
    } catch (err) {
      console.log(err);
    }
  }
  module.exports.imdb_crowler = imdb_crowler;